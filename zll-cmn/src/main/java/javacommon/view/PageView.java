...
import javacommon.page.PageUtils;
 ...
 * @author qy
 */
public class PageView<T> implements Body {
	...
	public PageView(PageRequest p, int totalCount) {
		this(p.getPageNumber(), p.getPageSize(), totalCount);
	}

	public PageView(int pageNumber, int pageSize, int totalCount) {
		this(pageNumber, pageSize, totalCount, new ArrayList(0));
	}

	public PageView(int pageNumber, int pageSize, int totalCount, List<T> result) {
		if (pageSize <= 0)
